...
//    Copyright (C) 2019 C.W.P. Schoenmakers
//
...
//
	fs_ll2uf2((uint8_t*)&mysir.sfdnc, fdnbc * 8 - 2);
        sscanf(fshex, "%4x", &ttybytes);
        mysir.ssidf = ttybytes >> 8 & 0xff;
        mysir.sdenf = ttybytes & 0xff;
//
...
int main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "b:f:s:n:F:")) != -1)
	{
		switch (opt)
		{
			case 'b':
			...
			case 's':
				...
				break;
                        case 'F' :
                                ...
                                break;
			default:
				fprintf(stderr, "usage: xxx -b #blocks -f <outputname> -n <diskname> -s #swap blocks> -F <SSDD>\n");
				exit(-1);
				break;
		}
	}
	...
	if (fsflag == 1)
        {
            if (strlen(fshex) != 4)
            {
		fprintf( stderr, "F option, incorrect string\n");
                exit (-1);
            }
            for (i = 0; i < strlen(fshex); i++)
            {
                if (isxdigit(fshex[i]) == 0)
                {
		    fprintf( stderr, "F option, incorrect string\n");
                    exit (-1);
                }
...
// zero out swap        
	printf("setup swap area\n");
	lseek(ofdn,(ulong)( sswpbg * DBLKSIZ) , 0);
	for (i = 0; i < sswpsz; i++)
	{
		add0blk(ofdn);
	}
